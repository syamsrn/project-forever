...
(function (global, factory) {
    ...
    class LevyPathGenerator {
        ...
        generate(p) {
            p.levy ??= {
                angle: engine.getRandom() * Math.PI * 2,
                baseSpeed: p.velocity.length,
            };
            const opts = this.options, l = p.levy;
            l.angle += (engine.getRandom() - 0.5) * 0.2;
            const u = Math.max(engine.getRandom(), 1e-6), scale = opts.scale ?? defaultScale, levyAlpha = opts.alpha ?? defaultLevyAlpha;
    ...
    exports.levyPathName = levyPathName;
    exports.loadLevyPath = loadLevyPath;

}));
Object.assign(globalThis.window || globalThis, { loadLevyPath: (globalThis.__tsParticlesInternals.paths.levy || {}).loadLevyPath });

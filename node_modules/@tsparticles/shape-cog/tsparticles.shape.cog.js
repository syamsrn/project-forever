...
(function (global, factory) {
    ...
    function drawCogHole(data) {
        ...
        const holeRadius = (radius * particle.cogHoleRadius) / engine.percentDenominator;
        context.globalCompositeOperation = "destination-out";
        context.beginPath();
        context.moveTo(holeRadius, engine.originPoint.y);
        context.arc(engine.originPoint.x, engine.originPoint.y, holeRadius, minAngle, engine.doublePI);
        context.closePath();
        context.fill();
    ...
    function drawCog(data) {
        ...
        const angle = engine.doublePI / (particle.cogNotches * engine.double), taperAI = angle * particle.cogInnerTaper * taperFactor, taperAO = angle * particle.cogOuterTaper * taperFactor, innerRadius = (radius * particle.cogInnerRadius) / engine.percentDenominator;
        let a = angle, toggle = false;
        context.moveTo(radius * Math.cos(taperAO), radius * Math.sin(taperAO));
        for (; a <= engine.doublePI; a += angle) {
            if (toggle) {
    ...
    class CogDrawer {
        ...
        particleInit(_container, particle) {
            ...
            particle.cogHoleRadius = engine.getRangeValue(shapeData?.holeRadius ?? defaultHoleRadius);
            particle.cogInnerRadius = engine.getRangeValue(shapeData?.innerRadius ?? defaultInnerRadius);
            particle.cogInnerTaper = engine.getRangeValue(shapeData?.innerTaper ?? defaultInnerTaper);
            particle.cogNotches = engine.getRangeValue(shapeData?.notches ?? defaultNotches);
            particle.cogOuterTaper = engine.getRangeValue(shapeData?.outerTaper ?? defaultOuterTaper);
        }
    }
...
Object.assign(globalThis.window || globalThis, { loadCogShape: (globalThis.__tsParticlesInternals.shapes.cog || {}).loadCogShape });

    ...
    class ParticlesDrawer {
        ...
        drawAfter(data) {
            ...
            particle.particlesNextSpawn =
                currentTime + (particle.particlesSpawnRate ?? defaultSpawnRate * engine.millisecondsToSeconds);
            const quantity = particle.particlesSpawnQuantity ?? defaultSpawnQuantity, pOptions = engine.deepExtend({}, particle.particlesData?.spawn?.particles), pos = { ...particle.getPosition() };
            for (let i = 0; i < quantity; i++) {
                container.particles.addParticle(pos, pOptions);
            }
        }
        particleInit(_container, particle) {
            const effectData = particle.effectData, spawnRate = engine.getRangeValue(effectData?.spawn?.rate?.delay ?? defaultSpawnRate);
            particle.particlesSpawnQuantity = engine.getRangeValue(effectData?.spawn?.rate?.quantity ?? defaultSpawnQuantity);
            particle.particlesSpawnRate = (spawnRate > minSpawnRate ? spawnRate : defaultSpawnRate) * engine.millisecondsToSeconds;
...
Object.assign(globalThis.window || globalThis, { loadParticlesEffect: (globalThis.__tsParticlesInternals.effects.particles || {}).loadParticlesEffect });

...
(function (global, factory) {
    ...
    class FilterDrawer {
        ...
        drawBefore(data) {
            ...
            context.save();
            const blurValue = typeof particle.filterBlur === "number" ? `${particle.filterBlur}px` : particle.filterBlur, brightnessValue = particle.filterBrightness, contrastValue = particle.filterContrast, dropShadowValue = particle.filterDropShadow, grayscaleValue = particle.filterGrayscale, hueRotateValue = typeof particle.filterHueRotate === "number" ? `${particle.filterHueRotate}deg` : particle.filterHueRotate, invertValue = particle.filterInvert, opacityValue = particle.filterOpacity, saturateValue = particle.filterSaturate, sepiaValue = particle.filterSepia, urlValue = particle.filterUrl, blurString = blurValue ? `blur(${blurValue})` : "", brightnessString = engine.isNull(brightnessValue) ? "" : `brightness(${brightnessValue})`, contrastString = engine.isNull(contrastValue) ? "" : `contrast(${contrastValue})`, dropShadowString = dropShadowValue ? `drop-shadow(${dropShadowValue})` : "", grayscaleString = engine.isNull(grayscaleValue) ? "" : `grayscale(${grayscaleValue})`, hueRotateString = engine.isNull(hueRotateValue) ? "" : `hue-rotate(${hueRotateValue})`, invertString = engine.isNull(invertValue) ? "" : `invert(${invertValue})`, opacityString = engine.isNull(opacityValue) ? "" : `opacity(${opacityValue})`, saturateString = engine.isNull(saturateValue) ? "" : `saturate(${saturateValue})`, sepiaString = engine.isNull(sepiaValue) ? "" : `sepia(${sepiaValue})`, urlString = urlValue ? `url(${urlValue})` : "";
            context.filter =
    ...
    exports.loadFilterEffect = loadFilterEffect;

}));
Object.assign(globalThis.window || globalThis, { loadFilterEffect: (globalThis.__tsParticlesInternals.effects.filter || {}).loadFilterEffect });

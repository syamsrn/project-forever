...
(function (global, factory) {
    ...
    class BackgroundMaskCover {
        ...
        load(data) {
            ...
            engine.loadProperty(this, "draw", data.draw);
            engine.loadProperty(this, "element", data.element);
            engine.loadProperty(this, "image", data.image);
    ...
    class BackgroundMask {
        ...
        load(data) {
            ...
            engine.loadProperty(this, "composite", data.composite);
            if (data.cover !== undefined) {
                const cover = data.cover, color = (engine.isString(data.cover) ? { color: data.cover } : data.cover);
    ...
    class BackgroundMaskPluginInstance {
        layer = engine.DrawLayer.BackgroundMask;
        #container;
        #coverColorStyle;
        #coverImage;
        ...
        canvasPaint() {
            ...
            if (maskElement) {
                ...
                canvas.render.draw(ctx => {
                    try {
                        ctx.drawImage(maskElement, engine.originPoint.x, engine.originPoint.y, canvas.size.width, canvas.size.height);
                    }
                    catch {
        ...
        #initCover = async () => {
            ...
            if (color) {
                const coverRgb = engine.rangeColorToRgb(this.#pluginManager, color);
                if (coverRgb) {
                    const coverColor = {
                        ...coverRgb,
                        a: cover.opacity,
                    };
                    this.#coverColorStyle = engine.getStyleFromRgb(coverColor, this.#container.hdr, coverColor.a);
                }
            }
            else if (cover?.image) {
                const coverImage = cover.image;
                await new Promise((resolve, reject) => {
                    const img = engine.safeDocument().createElement("img");
        ...
        #maskWarnOnce = (key, message) => {
            ...
            engine.getLogger().warning(`[tsParticles BackgroundMask] ${message}`);
        };
        #resolveMaskElement = () => {
...
Object.assign(globalThis.window || globalThis, { loadBackgroundMaskPlugin: (globalThis.__tsParticlesInternals.plugins.backgroundMask || {}).loadBackgroundMaskPlugin });

...
(function (global, factory) {
    ...
    class PolygonMaskDrawStroke {
        ...
        load(data) {
            ...
            if (engine.isString(this.color.value)) {
                this.opacity = engine.stringToAlpha(this.#pluginManager, this.color.value) ?? this.opacity;
            }
    ...
    })(exports.PolygonMaskInlineArrangement || (exports.PolygonMaskInlineArrangement = {}));

    class PolygonMaskInline {
        arrangement;
        constructor() {
            this.arrangement = exports.PolygonMaskInlineArrangement.onePerPoint;
        }
        load(data) {
            if (engine.isNull(data)) {
                return;
            }
            engine.loadProperty(this, "arrangement", data.arrangement);
        }
    }
    ...
    })(exports.PolygonMaskMoveType || (exports.PolygonMaskMoveType = {}));

    class PolygonMaskMove {
        radius = 10;
        type = exports.PolygonMaskMoveType.path;
        load(data) {
            if (engine.isNull(data)) {
                return;
            }
    ...
    })(exports.PolygonMaskType || (exports.PolygonMaskType = {}));
    ...
    class PolygonMask {
        ...
        position;
        scale = 1;
        type = exports.PolygonMaskType.none;
        url;
        constructor(pluginManager) {
        ...
        load(data) {
            ...
            if (data.enable !== undefined) {
                ...
                this.enable = this.type !== exports.PolygonMaskType.none;
            }
            engine.loadProperty(this, "url", data.url);
            if (data.data !== undefined) {
                if (engine.isString(data.data)) {
                    this.data = data.data;
                }
                else {
            ...
            if (data.position !== undefined) {
                this.position = engine.deepExtend({}, data.position);
            }
        }
    }
    ...
    class PolygonMaskPlugin {
        ...
        needsPlugin(options) {
            return (options?.polygon?.enable ??
                (options?.polygon?.type !== undefined && options.polygon.type !== exports.PolygonMaskType.none));
        }
    }
    ...
    function drawPolygonMask(pluginManager, context, rawData, stroke, hdr = false) {
        const color = engine.rangeColorToRgb(pluginManager, stroke.color);
        if (!color) {
            return;
        }
        ...
        context.closePath();
        context.strokeStyle = engine.getStyleFromRgb(color, hdr);
    ...
    function drawPolygonMaskPath(pluginManager, context, path, stroke, position, hdr = false) {
        ...
        context.strokeStyle = engine.getStyleFromRgb(color, hdr, stroke.opacity);
        context.lineWidth = stroke.width;
        context.stroke(path);
    ...
    function calcClosestPointOnSegment(s1, s2, pos) {
        const { dx: dx1, dy: dy1 } = engine.getDistances(pos, s1), { dx: dx2, dy: dy2 } = engine.getDistances(s2, s1), t = (dx1 * dx2 + dy1 * dy2) / (dx2 ** engine.squareExp + dy2 ** engine.squareExp), res = {
            x: s1.x + dx2 * t,
            y: s1.y + dy2 * t,
    ...
    function segmentBounce(start, stop, velocity) {
        const { dx, dy } = engine.getDistances(start, stop), wallAngle = Math.atan2(dy, dx), wallNormal = engine.Vector.create(Math.sin(wallAngle), -Math.cos(wallAngle)), d = engine.double * (velocity.x * wallNormal.x + velocity.y * wallNormal.y);
        wallNormal.multTo(d);
    ...
    class PolygonMaskInstance {
        ...
        clickPositionValid(position) {
            ...
            return (!!options?.enable &&
                options.type !== exports.PolygonMaskType.none &&
                options.type !== exports.PolygonMaskType.inline &&
        ...
        particlePosition(position) {
            ...
            return engine.deepExtend({}, position ?? this.#randomPoint());
        }
        particlesInitialization() {
            const options = this.#container.actualOptions.polygon;
            if (options?.enable &&
                options.type === exports.PolygonMaskType.inline &&
                (options.inline.arrangement === exports.PolygonMaskInlineArrangement.onePerPoint ||
                    options.inline.arrangement === exports.PolygonMaskInlineArrangement.perPoint)) {
                this.#drawPoints();
                return true;
            }
            return false;
        ...
        resize() {
            ...
            if (!(options?.enable && options.type !== exports.PolygonMaskType.none)) {
                return;
            }
        ...
        #checkInsidePolygon(position) {
            const container = this.#container, options = container.actualOptions.polygon;
            if (!options?.enable || options.type === exports.PolygonMaskType.none || options.type === exports.PolygonMaskType.inline) {
                return true;
            }
            if (!this.raw) {
                throw new Error(noPolygonFound);
            }
            const canvasSize = container.canvas.size, x = position?.x ?? engine.getRandom() * canvasSize.width, y = position?.y ?? engine.getRandom() * canvasSize.height, indexOffset = 1;
            let inside = false;
            ...
            if (options.type === exports.PolygonMaskType.inside) {
                return inside;
            }
            else {
                return !inside;
        ...
        #createPath2D() {
            ...
            for (const path of this.paths) {
                ...
                if (pathData) {
                    const path2d = new Path2D(pathData), matrix = engine.safeDocument().createElementNS("http://www.w3.org/2000/svg", "svg").createSVGMatrix(), finalPath = new Path2D(), transform = matrix.scale(this.#scale);
        ...
        #getEquidistantPointByIndex(index) {
            ...
            return {
                x: (point?.x ?? engine.originPoint.x) * scale + (this.offset?.x ?? engine.originPoint.x),
                y: (point?.y ?? engine.originPoint.y) * scale + (this.offset?.y ?? engine.originPoint.y),
            };
        }
        ...
        #getRandomPoint() {
            ...
            const coords = engine.itemFromArray(this.raw);
            if (!coords) {
                return;
            }
            return {
        ...
        #getRandomPointByLength() {
            ...
            const path = engine.itemFromArray(this.paths);
            if (!path) {
                return;
            }
            const offset = 1, distance = Math.floor(engine.getRandom() * path.length) + offset, point = path.element.getPointAtLength(distance), scale = this.#scale;
            return {
                x: point.x * scale + (this.offset?.x ?? engine.originPoint.x),
                y: point.y * scale + (this.offset?.y ?? engine.originPoint.y),
            };
        }
        async #initRawData(force) {
            ...
            if (options.url) {
                ...
                let svg;
                if (engine.isString(data)) {
                    svg = data;
                }
                else {
                    const getPath = (p) => `<path d="${p}" />`, path = engine.isArray(data.path) ? data.path.map(getPath).join("") : getPath(data.path), namespaces = 'xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"';
        ...
        #parseSvgPath(xml, force) {
            ...
            this.offset = {
                x: (canvasSize.width * position.x) / engine.percentDenominator - this.dimension.width * engine.half,
                y: (canvasSize.height * position.y) / engine.percentDenominator - this.dimension.height * engine.half,
            };
            return parsePaths(this.paths, scale, this.offset);
        }
        #polygonBounce(particle, _delta, direction) {
            const options = this.#container.actualOptions.polygon;
            if (!this.raw || !options?.enable || direction !== engine.OutModeDirection.top) {
                return false;
            }
            if (options.type === exports.PolygonMaskType.inside || options.type === exports.PolygonMaskType.outside) {
                let closest, dx, dy;
                ...
                for (let i = 0, j = this.raw.length - offset; i < this.raw.length; j = i++) {
                    ...
                    const dist = engine.getDistances(pos, closest);
                ...
                if (closest && dx !== undefined && dy !== undefined && !this.#checkInsidePolygon(pos)) {
                    const factor = { x: 1, y: 1 }, diameter = radius * engine.double, inverse = -1;
                    if (pos.x >= closest.x) {
                        factor.x = -1;
                    }
                    if (pos.y >= closest.y) {
            ...
            else if (options.type === exports.PolygonMaskType.inline) {
                const dist = engine.getDistance(particle.initialPosition, particle.getPosition()), { velocity } = particle;
                if (dist > this.#moveRadius) {
                    velocity.x = velocity.y * engine.half - velocity.x;
                    velocity.y = velocity.x * engine.half - velocity.y;
                    return true;
                }
            }
            return false;
        ...
        #randomPoint() {
            ...
            let position;
            if (options.type === exports.PolygonMaskType.inline) {
                switch (options.inline.arrangement) {
                    case exports.PolygonMaskInlineArrangement.randomPoint:
                        position = this.#getRandomPoint();
                        break;
                    case exports.PolygonMaskInlineArrangement.randomLength:
                        position = this.#getRandomPointByLength();
                        break;
                    case exports.PolygonMaskInlineArrangement.equidistant:
                        position = this.#getEquidistantPointByIndex(container.particles.count);
                        break;
                    case exports.PolygonMaskInlineArrangement.onePerPoint:
                    case exports.PolygonMaskInlineArrangement.perPoint:
                    default:
                ...
                position = {
                    x: engine.getRandom() * canvasSize.width,
                    y: engine.getRandom() * canvasSize.height,
                };
            }
...
Object.assign(globalThis.window || globalThis, { loadPolygonMaskPlugin: (globalThis.__tsParticlesInternals.plugins.polygonMask || {}).loadPolygonMaskPlugin });

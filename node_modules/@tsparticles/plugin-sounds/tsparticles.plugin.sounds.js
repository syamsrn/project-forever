...
(function (global, factory) {
    ...
    class SoundsAudio {
        ...
        load(data) {
            ...
            if (engine.isObject(data)) {
                engine.loadProperty(this, "loop", data.loop);
    ...
    class SoundsEvent {
        ...
        load(data) {
            ...
            engine.loadProperty(this, "event", data.event);
            if (data.audio !== undefined) {
                if (engine.isArray(data.audio)) {
            ...
            if (data.filter) {
                if (engine.isString(data.filter)) {
                    ...
                    if (engine.isFunction(filterFunc)) {
    ...
    class SoundsIcon {
        ...
        load(data) {
            ...
            engine.loadProperty(this, "svg", data.svg);
...
<svg baseProfile="tiny" height="24px" version="1.2" viewBox="0 0 24 24" width="24px"
    ...
    class SoundsVolume {
        ...
        load(data) {
            ...
            if (engine.isObject(data)) {
                ...
                engine.loadProperty(this, "step", data.step);
    ...
    function initImage(data) {
        const img = engine.safeDocument().createElement("img"), { clickCb, container, display, iconOptions, margin, options, pos, rightOffsets } = data, { width, path, style, svg } = iconOptions, defaultAccumulator = 0;
        ...
        const parent = container.canvas.domElement?.parentNode ?? engine.safeDocument().body;
        parent.append(img);
    ...
    class SoundsPluginInstance {
        ...
        async init() {
            ...
            for (const event of events) {
                ...
                const promises = engine.executeOnSingleOrMultiple(event.audio, async (audio) => {
                ...
                const cb = (args) => {
                    ...
                    void (async () => {
                        ...
                        if (!!this.#container.muted || this.#container.destroyed) {
                            engine.executeOnSingleOrMultiple(event.event, item => {
                        ...
                        if (event.audio) {
                            const audio = engine.itemFromSingleOrMultiple(event.audio);
                            if (!audio) {
                                return;
                            }
                            ...
                            const melody = engine.itemFromArray(event.melodies);
                            if (!melody) {
                                return;
                            }
                            ...
                            const note = engine.itemFromArray(event.notes);
                            if (!note) {
                                return;
                            }
        ...
        async #playNote(notes, noteIdx, loop) {
            ...
            const value = note.value, promises = engine.executeOnSingleOrMultiple(value, async (_, idx) => {
                return this.#playNoteValue(notes, noteIdx, idx);
            });
            await (engine.isArray(promises) ? Promise.allSettled(promises) : promises);
            const indexOffset = 1;
        ...
        async #playNoteValue(notes, noteIdx, valueIdx) {
            ...
            const value = engine.itemFromSingleOrMultiple(note.value, valueIdx, true);
            if (!value) {
                return;
            }
            try {
                const freq = getNoteFrequency(value);
                if (!engine.isNumber(freq)) {
                    return;
                }
        ...
        #unmute() {
            ...
            const audioContext = this.#getAudioContext(), gain = audioContext.createGain();
            gain.connect(audioContext.destination);
            gain.gain.value = soundsOptions.volume.value / engine.percentDenominator;
            this.#gain = gain;
            this.#initEvents();
        ...
        async #updateVolume() {
            ...
            engine.clamp(this.#volume, soundsOptions.volume.min, soundsOptions.volume.max);
            ...
            if (this.#gain?.gain) {
                this.#gain.gain.value = this.#volume / engine.percentDenominator;
            }
        }
    }
...
Object.assign(globalThis.window || globalThis, { loadSoundsPlugin: (globalThis.__tsParticlesInternals.plugins.sounds || {}).loadSoundsPlugin });

...
(function (global, factory) {
    ...
    const dirs = [
        engine.Vector.create(engine.identity, engine.originPoint.y),
        engine.Vector.create(engine.originPoint.x, engine.identity),
        engine.Vector.create(-engine.identity, engine.originPoint.y),
        engine.Vector.create(engine.originPoint.x, -engine.identity),
    ...
    class GridPathGenerator {
        ...
        generate(p) {
            ...
            p.grid ??= {
                direction: Math.trunc(engine.getRandom() * dirs.length),
            ...
            if (c.x !== grid.cellPosition.x || c.y !== grid.cellPosition.y) {
                ...
                if (allowed && allowed.length > minLength) {
                    grid.direction = allowed[Math.trunc(engine.getRandom() * allowed.length)];
                }
                else if (!this.options.graph) {
                    const turn = Math.trunc(engine.getRandom() * 3) - 1;
        ...
        #generateMazeGraph({ height, width }) {
            ...
            const stack = [{ x: engine.originPoint.x, y: engine.originPoint.y }];
            while (stack.length > 0) {
                const { x, y } = stack[stack.length - 1];
                if (!visited[y][x]) {
                    visited[y][x] = true;
                }
                const dirsOrder = [0, 1, 2, 3].sort(() => engine.getRandom() - 0.5);
                let moved = false;
    ...
    exports.gridPathName = gridPathName;
    exports.loadGridPath = loadGridPath;

}));
Object.assign(globalThis.window || globalThis, { loadGridPath: (globalThis.__tsParticlesInternals.paths.grid || {}).loadGridPath });

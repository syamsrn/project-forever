...
(function (global, factory) {
    ...
    function hsvToRgb(hsv) {
        const result = { b: 0, g: 0, r: 0 }, phase = 60, hsvPercent = {
            h: hsv.h / phase,
            s: hsv.s / engine.percentDenominator,
            v: hsv.v / engine.percentDenominator,
        ...
        if (tempRgb) {
            ...
            result.r = Math.floor((tempRgb.r + m) * engine.rgbMax);
            result.g = Math.floor((tempRgb.g + m) * engine.rgbMax);
            result.b = Math.floor((tempRgb.b + m) * engine.rgbMax);
        }
        return result;
    }
    ...
    class HsvColorManager {
        ...
        handleRangeColor(color) {
            ...
            return hsvToRgb({
                h: engine.getRangeValue(hsvColor.h),
                s: engine.getRangeValue(hsvColor.s),
                v: engine.getRangeValue(hsvColor.v),
            });
        }
        parseString(input) {
            ...
            return result
                ? hsvaToRgba({
                    a: result.length > fullLength ? engine.parseAlpha(result[indexes.a]) : defaultAlpha,
...
Object.assign(globalThis.window || globalThis, { loadHsvColorPlugin: (globalThis.__tsParticlesInternals.plugins.hsvColor || {}).loadHsvColorPlugin });

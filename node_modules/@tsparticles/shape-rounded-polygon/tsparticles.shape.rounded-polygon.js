...
(function (global, factory) {
    ...
    function polygon(sides, radius, rot = defaultRotation) {
        const step = engine.doublePI / sides, path = [];
    ...
    function roundedPath(context, path, radius) {
        ...
        context.moveTo((p1.x + p2.x) * engine.half, (p1.y + p2.y) * engine.half);
        for (let i = 1; i <= len; i++) {
            p1 = p2;
            ...
            context.arcTo(p1.x, p1.y, (p1.x + p2.x) * engine.half, (p1.y + p2.y) * engine.half, radius);
        }
    }
    ...
    class RoundedPolygonDrawer {
        ...
        getSidesCount(particle) {
            ...
            return Math.round(engine.getRangeValue(roundedPolygon?.sides ?? defaultSides));
        }
        particleInit(container, particle) {
            const shapeData = particle.shapeData;
            particle.borderRadius = Math.round(engine.getRangeValue(shapeData?.radius ?? defaultSides)) * container.retina.pixelRatio;
        }
    }
...
Object.assign(globalThis.window || globalThis, { loadRoundedPolygonShape: (globalThis.__tsParticlesInternals.shapes.roundedPolygon || {}).loadRoundedPolygonShape });
